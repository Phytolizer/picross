...
import mygl.Alignment;
import mygl.ButtonElement;
import mygl.ElementArray;
...
public class GameWindow extends mygl.Graphics {
    ...
    private List<ButtonElement> controlsButtons;
    ...
    //region ButtonElement categories
    private ElementArray mainMenuButtons;
    ...
    //endregion
    //region buttons
    public ButtonElement bPause;
    private ButtonElement bResume;
    private ButtonElement bNewPuzzle;
    private ButtonElement bLeaderboard;
    private ButtonElement bXUp;
    private ButtonElement bXDown;
    private ButtonElement bYUp;
    private ButtonElement bYDown;
    private ButtonElement bBack;
    private ButtonElement bStart;
    private ButtonElement bMainMenu;
    private ButtonElement bMainMenu2;
    private ButtonElement bQuitGame;
    private ButtonElement bBegin;
    private ButtonElement bRegenPuzzle;
    private ButtonElement bControlsMenu;
    private ButtonElement bCreator;
    private ButtonElement bRandomPuzzle;
    private ButtonElement bLoadPuzzle;
    private ButtonElement bRestoreControls;
    public ButtonElement bGamba;
    //endregion
    //region check boxes
    ...
    public GameWindow() {
        ...
        initButtons();
        initControls();
        competitiveModeToggle = new CheckBox(frameWidth - 35, frameHeight - 35, 25, false);
    ...
    public void submitScore() {
        ...
        try {
            url = new URL("https://westonreed.com/picross/addscore.php?username=" + username
                          + "&password=" + password + "&time=" + time + "&size=" + sizeX + "x"
                          + sizeY + "&version=" + VERSION);
            url.openStream();
        } catch (IOException e) {
            ...
            try {
                ...
                url = new URL("http://westonreed.com/picross/addscore.php?username=" + username
                              + "&password=" + password + "&time=" + time + "&size=" + sizeX + "x"
                              + sizeY + "&version=" + VERSION);
                url.openStream();
    ...
    protected void update() {
        ...
        int i;
        if (currWindow == GAME) {
            // TODO delegate these to ButtonElement Updaters
            if (playable /*&& !competitiveMode*/) {
                bPause.setVisible(true);
                
            } else {
                bBegin.setPos(frameWidth / 2 - 100, frameHeight / 2 - 50);
                bResume.setPos(frameWidth / 2 - 100, frameHeight / 2 + 7);
                bMainMenu.setPos(frameWidth / 2 - 100, frameHeight / 2 + 7);
                bMainMenu2.setPos(frameWidth / 2, frameHeight / 2 + 7);
                bRegenPuzzle.setPos(frameWidth / 2, frameHeight / 2 + 7);
                bCreator.setPos(frameWidth / 2 - 100, bCreator.getY());
            }
            ...
            bRandomPuzzle.setPos(frameWidth / 2 - 100, bRandomPuzzle.getY());
            bLoadPuzzle.setPos(frameWidth / 2 - 100, bLoadPuzzle.getY());
        } else if (currWindow == SIZE_PICKER) {
            frame.setMinimumSize(new Dimension(
                    getStrLen("SIZE PICKER", 50f) + bBack.getSize().width * 2 + 25,
                    100 + 50 + 10 + 50 + 60 + 250
            ));
            int freeSpace = frameHeight - 100 - bStart.getSize().height - 150;
            bXUp.setVisible(true);
            bXUp.setPos(frameWidth / 2 - 200, freeSpace / 2 - 55 + 160);
            bXDown.setVisible(true);
            bXDown.setPos(bXUp.getX(), bXUp.getY() + 110);
            bYUp.setVisible(true);
            bYUp.setPos(frameWidth / 2 + 100, freeSpace / 2 - 55 + 160);
            bYDown.setVisible(true);
            bYDown.setPos(bYUp.getX(), bYUp.getY() + 110);
            bBack.setVisible(true);
            bStart.setVisible(true);
            bStart.setPos(frameWidth / 2 - 50, frameHeight - 100);
            if (sizeX > 25) {
                sizeX = 25;
            }
            if (sizeX < 1) {
            ...
            bNewPuzzle.setPos(frameWidth / 2 - 100, bNewPuzzle.getY());
            bLeaderboard.setPos(frameWidth / 2 - 100, bLeaderboard.getY());
            bQuitGame.setPos(frameWidth / 2 - 100, bQuitGame.getY());
            bControlsMenu.setPos(frameWidth / 2 - 100, bControlsMenu.getY());
            bCreator.setPos(frameWidth / 2 - 100, bCreator.getY());
            competitiveModeToggle.setPos(frameWidth - 35, frameHeight - 35);
    ...
    protected void draw() {
        switch (currWindow) {
            case MENU:
                ...
                DrawingTools.drawCenteredText(f, "MAIN MENU", frameWidth / 2, 100, graphics2D);
                graphics2D = setFont(20f, graphics2D);
                graphics2D.drawString("v" + VERSION, 15, frameHeight - 15);
                competitiveModeToggle.draw(mouseX, mouseY, graphics2D);
                graphics2D.setColor(black);
                DrawingTools.drawRightText(f, "Competitive Mode",
                        frameWidth - 35, frameHeight - 15, graphics2D);
                break;
            case OPTIONS:
                graphics2D.setColor(bgColor);
                graphics2D.fillRect(0, 0, frameWidth, frameHeight);
                graphics2D.setColor(BLACK);
                graphics2D = setFont(50f, graphics2D);
                DrawingTools.drawCenteredText(f, "OPTIONS", frameWidth / 2, 100, graphics2D);
                break;
            case GAMEMODE:
                graphics2D.setColor(bgColor);
                graphics2D.fillRect(0, 0, frameWidth, frameHeight);
                graphics2D.setColor(BLACK);
                graphics2D = setFont(50f, graphics2D);
                DrawingTools.drawCenteredText(f, "CHOOSE GAMEMODE",
                        frameWidth / 2, 100, graphics2D);
                break;
            case SIZE_PICKER:
                ...
                } else if (sizeX == 1 || (modifier && sizeX - 5 < 1) || (competitiveMode && (
                        sizeX <= 5 || modifier && sizeX - 5 < 5))) {
                ...
                } else if (sizeY == 1 || (modifier && sizeY - 5 < 1) || (competitiveMode && (
                        sizeY <= 5 || modifier && sizeY - 5 < 5))) {
                ...
                graphics2D.setColor(BLACK);
                graphics2D = setFont(50f, graphics2D);
                DrawingTools.drawCenteredText(f, "SIZE PICKER", frameWidth / 2, 100, graphics2D);
                DrawingTools.drawCenteredText(f, "X",
                        bXUp.getX() + bXUp.getSize().width / 2, bXUp.getY() - 10, graphics2D);
                DrawingTools.drawCenteredText(f, "Y",
                        bYUp.getX() + bYUp.getSize().width / 2, bYUp.getY() - 10, graphics2D);
                DrawingTools.drawCenteredText(f, Integer.toString(sizeX),
                        bXUp.getX() + bXUp.getSize().width / 2, bXDown.getY() - 10, graphics2D);
                DrawingTools.drawCenteredText(f, Integer.toString(sizeY),
                        bYUp.getX() + bYUp.getSize().width / 2, bYDown.getY() - 10, graphics2D);
                graphics2D = setFont(20f, graphics2D);
                break;
            case GAME:
                ...
                graphics2D.setColor(BLACK);
                cWidth = frameWidth / 2 - (gameGrid.sizeX * bSize / 2) - clueLen[0];
                if (cWidth < 0) {
                    cWidth = 0;
                }
                for (int i = 0; i < (gameGrid.sizeX); i++) {
                    for (int j = 0; j < (gameGrid.sizeY); j++) {
                        ...
                        if (playable && currBox != null && i == currBox.getPos()[0]
                            && j == currBox.getPos()[1]) {
                            graphics2D.setColor(new Color(0, 0, 0, 64));
                            graphics2D.fillRect(
                                    clueLen[0] + i * bSize + cWidth,
                                    clueLen[1] + j * bSize, bSize, bSize);
                        ...
                        } else if (playable && ((i == (mouseX - clueLen[0] - cWidth) / bSize
                                                 && mouseX > clueLen[0] + cWidth) || (
                                                        j == (mouseY - clueLen[1]) / bSize
                                                        && mouseY > clueLen[1]))) {
                            graphics2D.setColor(new Color(0, 0, 0, 32));
                            graphics2D.fillRect(
                                    clueLen[0] + i * bSize + cWidth,
                ...
                for (int i = 5; i < gameGrid.sizeX; i += 5) {
                    graphics2D.drawLine(
                            clueLen[0] + i * bSize + 1 + cWidth, clueLen[1],
                            clueLen[0] + i * bSize + 1 + cWidth,
                            clueLen[1] + gameGrid.sizeY * bSize);
                }
                for (int i = 5; i < gameGrid.sizeY; i += 5) {
                    graphics2D.drawLine(
                            clueLen[0] + cWidth,
                            clueLen[1] + i * bSize + 1,
                            clueLen[0] + gameGrid.sizeX * bSize + cWidth,
                            clueLen[1] + i * bSize + 1);
                }
                graphics2D = setFont(20f, graphics2D);
                graphics2D.drawString("MISTAKES: ", 10, frameHeight - 15);
                int xRendered = 0,
                        mistakesTemp = numMistakes;
                graphics2D.drawRect(120, frameHeight - 35, 125, 25);
                graphics2D.setColor(RED);
                while (mistakesTemp > 0 && xRendered < 5) {
                    graphics2D.drawString("X", xRendered * 25 + 125, frameHeight - 15);
                    mistakesTemp--;
                    xRendered++;
                }
                while (xRendered < 5) {
                ...
                if (!playable) {
                    ...
                    graphics2D.fillRect(frameWidth / 2 - 100, frameHeight / 2 - 50, 200, 100);
                    graphics2D.setColor(BLACK);
                    graphics2D.drawRect(frameWidth / 2 - 100, frameHeight / 2 - 50, 200, 100);
                    String showText = "";
                    ...
                    switch (status) {
                        ...
                        case "paused":
                            if (Main.animator.getMS() % 1000 <= 500) {
                                DrawingTools.drawCenteredText(f, "PAUSED",
                                        frameWidth / 2, frameHeight / 2 - 10, graphics2D);
                            }
                    ...
                    graphics2D = setFont(30f, graphics2D);
                    DrawingTools.drawCenteredText(f, showText,
                            frameWidth / 2, frameHeight / 2 - 10, graphics2D);
                    graphics2D.setColor(BLACK);
                    //if(!status.equals("get ready") && !status.equals("paused"))
                    //graphics2D.drawString("TIME:" + Main.timer.toString(), frameWidth / 2 - 45, frameHeight / 2 - 12);
                }
                //render mistakes/timer
                graphics2D = setFont(20f, graphics2D);
                graphics2D.setColor(BLACK);
                DrawingTools.drawRightText(f,
                        "TIME: " + Main.timer.toString(false), frameWidth,
                        frameHeight - 15, graphics2D);
                if (showingPausePrompt) {
                    graphics2D = setFont(20f, graphics2D);
                    graphics2D.drawString("Press the pause ButtonElement again to confirm disabling Competitive Mode",
                            bPause.getX() + bPause.getSize().width + 15,
                            bPause.getY() + bPause.getSize().height / 2 + 10);
                }
                break;
            case CONTROLS:
                graphics2D.setColor(bgColor);
                ...
                graphics2D.setColor(black);
                graphics2D = setFont(50f, graphics2D);
                DrawingTools.drawCenteredText(f, "CONTROLS", frameWidth / 2, 100, graphics2D);
                graphics2D.drawRect(100, 150, frameWidth - 200, frameHeight - 250);
                frame.setMinimumSize(new Dimension(
                        100 + 100 + 10 + getMaxStrLen(controlsDescriptions, 25f) + 100,
                        150 + (50 * (controlsMenuButtons.size() - 2)) + 100));
                graphics2D = setFont(25f, graphics2D);
                for (int i = 0; i < controlsDescriptions.size(); i++) {
                    graphics2D.drawString(controlsDescriptions.get(i), 210,
                            150 + 50 * (i + 1) - 15);
                }
                break;
            case LOAD:
                ...
                graphics2D.setColor(BLACK);
                graphics2D = setFont(50f, graphics2D);
                DrawingTools.drawCenteredText(f, "LOAD A PUZZLE", frameWidth / 2, 100, graphics2D);
    ...
    private int getBoxSize() {
        int temp =
                (frameWidth - (clueLen[0] > clueLen[1] ? clueLen[0] : clueLen[1])) / (gameGrid.sizeX
                                                                                      + 1);
        if (temp * gameGrid.sizeY + clueLen[1] + 50 > frameHeight) {
            return (frameHeight - 50 - clueLen[1]) / gameGrid.sizeY > MIN_BSIZE ?
                   (frameHeight - 50 - clueLen[1]) / gameGrid.sizeY : MIN_BSIZE;
        } else {
     ...
     * ButtonElement class. Includes gameplay and scrolling on the size picker.
     */
        ...
        switch (currWindow) {
            case GAME:
                if (controlMode == ControlMode.MOUSE) {
                    ...
                    if (currBox != null && (mouseX - clueLen[0] - cWidth) / bSize < gameGrid.sizeX
                        && (mouseY - clueLen[1]) / bSize < gameGrid.sizeY
                        && mouseX > clueLen[0] + cWidth && mouseY > clueLen[1]
                        && currBox != gameGrid.getBox(
                            (mouseX - clueLen[0] - cWidth) / bSize,
                            (mouseY - clueLen[1]) / bSize)) {
                        currBox.setCanModify(true);
                    }
                    //get box only if mouse is within game grid, otherwise it is null
                    if ((mouseX - clueLen[0] - cWidth) / bSize < gameGrid.sizeX
                        && (mouseY - clueLen[1]) / bSize < gameGrid.sizeY
                        && mouseX > clueLen[0] + cWidth && mouseY > clueLen[1]) {
                        currBox = gameGrid.getBox(
                                (mouseX - clueLen[0] - cWidth) / bSize,
            ...
            case SIZE_PICKER:
                if (frame.scrollAmt != 0) {
                    if (isInBounds(bXUp.getX(),
                            bXUp.getY() + bXUp.getSize().height,
                            bXUp.getX() + bXUp.getSize().width,
                            bXUp.getY() + bXUp.getSize().height + 60)) {
                        int min = competitiveMode ? 4 : 0;
                        if (modifier) {
                            sizeX -= sizeX - (5 * frame.scrollAmt) > min
                                     && sizeX - (5 * frame.scrollAmt) <= 25 ?
                                     frame.scrollAmt * 5 : 0;
                        } else {
                            sizeX -= sizeX - frame.scrollAmt > min
                                     && sizeX - frame.scrollAmt <= 25 ? frame.scrollAmt : 0;
                        }
                    } else if (isInBounds(bYUp.getX(),
                            bYUp.getY() + bYUp.getSize().height,
                            bYUp.getX() + bYUp.getSize().width,
                            bYUp.getY() + bYUp.getSize().height + 60)) {
                        int min = competitiveMode ? 4 : 0;
                        if (modifier) {
                            sizeY -= sizeY - (5 * frame.scrollAmt) > min
                                     && sizeY - (5 * frame.scrollAmt) <= 25 ?
                                     frame.scrollAmt * 5 : 0;
                        } else {
                            sizeY -= sizeY - frame.scrollAmt > min
                                     && sizeY - frame.scrollAmt <= 25 ? frame.scrollAmt : 0;
                        }
                    }
                    frame.scrollAmt = 0;
                }
            ...
            case LOAD:
                if (frame.scrollAmt != 0) {
                    scrollIndex += (scrollIndex + frame.scrollAmt >= 0
                                    && scrollIndex + frame.scrollAmt
                                       <= puzzleButtons.size() - 5 ? frame.scrollAmt : 0);
    ...
    private void generatePuzzle() {
        ...
        do {
            ...
            numSolutions = Integer.parseInt(output.get(solutionsLine).substring(
                    output.get(solutionsLine).length() - 1, output.get(solutionsLine).length()));
        ...
        System.out.println(
                "Generated puzzle in " + numTries + " " + (numTries == 1 ? "try." : "tries."));
    ...
    /**
     * Performs a predetermined action based on the ButtonElement passed.
     *
     * @param b ButtonElement to be compared with known buttons
     */
    void doClickAction(ButtonElement b) {
        if (b != bXUp && b != bLeaderboard && b != bXDown && b != bYUp && b != bYDown && b != bBegin
            && b != bPause && !(b instanceof ControlsButton)) {
        ...
        if (b == bNewPuzzle) {
            ...
            scrollIndex = 0;
            ButtonElement[] pButtons = new ButtonElement[getNumPuzzles()];
            for (int i = 0; i < getNumPuzzles(); i++) {
                pButtons[i] = new ButtonElement(this);
                pButtons[i].setText(puzzleNames.get(i).substring(0,
                        puzzleNames.get(i).length() - 4));
            }
            ...
            HashMap<String, ButtonElement> controlsButtons = new HashMap<>();
            for (ButtonElement b1 : controlsMenuButtons.toList()) {
                if (b1 instanceof ControlsButton) {
                    controlsButtons.put(((ControlsButton) b1).getLabel(), b1);
                }
            }
            if (pkh.getKeyAssigning() != null) {
                //return previously assigning ButtonElement's key to normal
                updateButtons("controls");
            }
    ...
    private void initButtons() {
        ...
        mainMenuButtons = new ElementArray(graphics2D);
        bNewPuzzle = new ButtonElement(this);
        bNewPuzzle.resize(200, 65);
        bNewPuzzle.setBackgroundColor(Color.green);
        bNewPuzzle.setText("Start Game");
        bNewPuzzle.setUpdater(() -> bNewPuzzle.moveX(frameWidth / 2, Alignment.CENTER));
        bLeaderboard = new ButtonElement(this);
        bLeaderboard.resize(200, 65);
        bLeaderboard.setBackgroundColor(Color.yellow);
        bLeaderboard.setText("Leaderboard");
        bLeaderboard.setUpdater(() -> bLeaderboard.moveX(frameWidth / 2, Alignment.CENTER));
        bCreator = new ButtonElement(this);
        bCreator.resize(200, 65);
        bCreator.setBackgroundColor(new Color(255, 140, 0));
        bCreator.setText("Creator");
        bCreator.setUpdater(() -> bCreator.moveX(frameWidth / 2, Alignment.CENTER));
        bControlsMenu = new ButtonElement(this);
        bControlsMenu.resize(200, 65);
        bControlsMenu.setBackgroundColor(new Color(0, 50, 255));
        bControlsMenu.setText("Controls");
        bControlsMenu.setUpdater(() -> bControlsMenu.moveX(frameWidth / 2, Alignment.CENTER));
        bQuitGame = new ButtonElement(
                frameWidth / 2 - 100, 450, 200, 65, "Quit Game", RED, 20, this);
        bQuitGame.setVisible(true);
        mainMenuButtons.setElements(new ButtonElement[]{bNewPuzzle, bLeaderboard, bCreator, bControlsMenu, bQuitGame});
        
        gameChoiceButtons = new ButtonList(GAMEMODE);
        bBack = new ButtonElement(10, 55, 50, 50, "<", RED, 30, this);
        bRandomPuzzle = new ButtonElement(
                frameWidth / 2 - 100, 150, 200, 100, "Random Puzzle", GREEN, 20, this);
        bLoadPuzzle = new ButtonElement(
                frameWidth / 2 - 100, 275, 200, 100, "Load Puzzle", YELLOW, 20, this);
        gameChoiceButtons.addButtons(new ButtonElement[]{bRandomPuzzle, bLoadPuzzle, bBack});
        
        loadMenuButtons = new ButtonList(LOAD);
        loadMenuButtons.addButtons(new ButtonElement[]{bBack});
        
        puzzleButtons = new ButtonList(LOAD);
        
        sizePickerButtons = new ButtonList(SIZE_PICKER);
        bXUp = new ButtonElement(300, 400, 100, 50, "Λ", 30, this);
        bXDown = new ButtonElement(300, 510, 100, 50, "V", 30, this);
        bYUp = new ButtonElement(600, 400, 100, 50, "Λ", 30, this);
        bYDown = new ButtonElement(600, 510, 100, 50, "V", 30, this);
        bStart = new ButtonElement(
                frameWidth / 2 - 50, frameHeight - 100, 100, 75, "GENERATE", GREEN, 30, this);
        sizePickerButtons.addButtons(new ButtonElement[]{bXUp, bXDown, bYUp, bYDown, bBack, bStart});
        
        optionsMenuButtons = new ButtonList(OPTIONS);
        optionsMenuButtons.addButtons(new ButtonElement[]{bBack});
        
        gameButtons = new ButtonList(GAME);
        bPause = new ButtonElement(20, 50, 60, 60, "Pause", YELLOW, 17, this);
        gameButtons.addButton(bPause);
        
        pauseMenuButtons = new ButtonList(PAUSE);
        bResume = new ButtonElement(
                frameWidth / 2 - 100, frameHeight / 2 + 7, 100, 43, "Resume", GREEN, 17, this);
        bMainMenu = new ButtonElement(
                frameWidth / 2 - 100, frameHeight / 2 + 7, 100, 43, "Main Menu", bgColor, 17, this);
        bMainMenu2 = new ButtonElement(
                frameWidth / 2, frameHeight / 2 + 7, 100, 43, "Main Menu", bgColor, 17, this);
        bRegenPuzzle = new ButtonElement(
                frameWidth / 2, frameHeight / 2 + 7, 100, 43, "New Puzzle", GREEN, 17, this);
        bBegin = new ButtonElement(
                frameWidth / 2 - 100, frameHeight / 2 - 50, 200, 100, "BEGIN", GREEN, 20, this);
        pauseMenuButtons.addButtons(new ButtonElement[]{bResume/*, bMainMenu*/, bMainMenu2/*, bRegenPuzzle*//*, bBegin*/});
        
        gameEndButtons = new ButtonList(GAME_END);
        gameEndButtons.addButtons(new ButtonElement[]{bMainMenu, bRegenPuzzle});
        
        controlsMenuButtons = new ButtonList(CONTROLS);
        bRestoreControls = new ButtonElement(
                frameWidth - 150 - 10, 55, 150, 50, "Restore Defaults", YELLOW, 20, this);
        controlsMenuButtons.addButtons(new ButtonElement[]{bBack, bRestoreControls});
    ...
    private void initControls() {
        ...
        if (!(prefs.has("pauseGame") && prefs.has("up") && prefs.has("left") && prefs.has("down")
              && prefs.has("right") && prefs.has("resolve1") && prefs.has("resolve2"))) {
            try {
                newControls = true;
    ...
    private void displayStatus(String message) {
        ...
        art.fillRect(0, 0, frameWidth, frameHeight);
        art.setColor(black);
        art = setFont(50f, art);
        DrawingTools.drawCenteredText(f, message, frameWidth / 2, frameHeight / 2 + 25, art);
        art = (Graphics2D) frame.getGraphics();
        if (art != null) {
            art.drawImage(imgBuffer, 0, 0, frameWidth, frameHeight, 0, 0, frameWidth, frameHeight, null);
            art.dispose();
        }
    }
